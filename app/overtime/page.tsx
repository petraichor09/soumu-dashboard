...
import { createOvertimeRequest, updateOvertimeStatus, deleteOvertimeRequest } from '@/app/actions'
...
export default async function OvertimePage() {
  const requests = await prisma.overtimeRequest.findMany({ orderBy: { createdAt: 'desc' } })
  ...
  return (
    <div>
      ...
      <div className="card mb-6" style={{ padding: '1.5rem' }}>
        ...
        <form action={createOvertimeRequest}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.875rem', marginBottom: '1rem' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 600, color: '#475569', marginBottom: '6px' }}>
                残業日 <span style={{ color: '#ef4444' }}>*</span>
              </label>
            ...
            <div>
              ...
              <input name="startTime" type="time" required className="form-input" />
            </div>
            <div>
              <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 600, color: '#475569', marginBottom: '6px' }}>
                終了時刻 <span style={{ color: '#ef4444' }}>*</span>
              </label>
              <input name="endTime" type="time" required className="form-input" />
            </div>
            <div style={{ gridColumn: 'span 2' }}>
              <label style={{ display: 'block', fontSize: '0.78rem', fontWeight: 600, color: '#475569', marginBottom: '6px' }}>
                理由 <span style={{ color: '#ef4444' }}>*</span>
              </label>
              <input name="reason" placeholder="例：月次締め対応" required className="form-input" />
            </div>
          </div>
              ...
              {requests.map((r) => {
                ...
                return (
                  <tr key={r.id}>
                    ...
                    <td style={{ color: '#64748b' }}>{r.startTime} 〜 {r.endTime}</td>
                    ...
                    <td>
                      <div style={{ display: 'flex', gap: '6px', alignItems: 'center' }}>
                        {r.status === 'pending' && (
                          <>
                            <form action={updateOvertimeStatus.bind(null, r.id, 'approved')}>
                            ...
                            <form action={updateOvertimeStatus.bind(null, r.id, 'rejected')}>
                        ...
                        <form action={deleteOvertimeRequest.bind(null, r.id)}>
